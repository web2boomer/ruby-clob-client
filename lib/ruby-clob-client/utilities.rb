...
module RubyClobClient
  module Utilities
    def self.parse_raw_orderbook_summary(raw_obs)
      bids = raw_obs["bids"].map { |bid| RubyClobClient::OrderSummary.new(size: bid["size"], price: bid["price"]) }
      asks = raw_obs["asks"].map { |ask| RubyClobClient::OrderSummary.new(size: ask["size"], price: ask["price"]) }
      RubyClobClient::OrderBookSummary.new(
